...
```{r Alpha diversity statistics, message=F, echo=F, eval=F, warning=F, include=F, cache=F, fig.height=8, fig.width=4}
...
alpha %>% with(., lm(Chao1 ~ Filter_type + Sample_type + Protocol)) %>% 
  anova %>% 
  broom::tidy() %>% 
  kableExtra::kable(caption = "Chao1 richness  statistics", digits = 3) %>%  kableExtra::kable_classic()


alpha %>% with(., lm(Shannon ~ Filter_type + Sample_type + Protocol)) %>% 
  anova %>% 
  broom::tidy() %>% 
  kableExtra::kable(caption = "Shannon diversity statistics", digits = 3) %>%  kableExtra::kable_classic()
...
### NOT RUN because of low power (n=3 per group including mixture sample)
# ## Before vs After in Granules only
# # Chao1 pairwise
# alpha_time_chao1 <- 
# alpha %>% 
#   filter(Protocol == "Fast_DNA",
#          Sample_type == "Granules") %>% 
#   with(., lm(Chao1 ~ Time*Filter_type)) 
# 
# alpha_time_chao1 %>% 
#   anova() %>% 
#   broom::tidy() %>% 
#   kableExtra::kable(caption = "Difference Chao1 richness between before and after backwashing of granules per BODAC 1 and 2", digits = 3) %>%  kableExtra::kable_classic()
# 
# phia::testInteractions(alpha_time_chao1, fixed = "Filter_type", pairwise = "Time", adjustment = "fdr")
# 
# # shannon pairwise
# alpha_time_shannon <-
# alpha %>% 
#   filter(Protocol == "Fast_DNA",
#          Sample_type == "Granules") %>% 
#   with(., lm(Shannon ~ Time*Filter_type)) 
# 
# alpha_time_shannon %>% 
#   anova() %>% 
#   broom::tidy() %>% 
#   kableExtra::kable(caption = "Difference Shannon diversity between before and after backwashing of granules per BODAC 1 and 2", digits = 3) %>%  kableExtra::kable_classic()
# 
# phia::testInteractions(alpha_time_shannon, fixed = "Filter_type", pairwise = "Time", adjustment = "fdr")

```

```{r Breakaway, eval=F, echo=F, include=F}
